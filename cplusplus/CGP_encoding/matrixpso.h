...
struct MatrixDecoder {
    ...
    /* Evaluation temporaries (bit-packed, 64 truth-table rows per word).
       Both buffers are flat: indexed as [wire * numWords + word].
       - inTT  : signal values entering the current column
       - output: signal values produced by the current column    */
    std::vector<uint64_t> inTT;
