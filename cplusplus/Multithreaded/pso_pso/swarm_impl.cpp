...
/* =========================================================
 * PSO-PSO extensions (Gonsalves & Egashira, 2013)
 * See declarations in psomatrixcircuit.h for the algorithm
 * this implements; orchestration across sub-swarms and
 * threads lives in psopso.h/.cpp.
 * ========================================================= */

/* Size sBest and make sure it loses every first comparison */
void Swarm::initSwarmBest()
{
    reserveParticleMemory(sBest, true);
    sBest.fitness = -std::numeric_limits<double>::infinity();
}

/* Evaluate every particle, update pbest (bestIndividualExp),
   this swarm's sBest, and Gen statistics. Unlike
   evaluatePopulation(), there is no ring-neighbourhood scan --
   the "social" experience shared by every particle in the
   swarm is simply sBest. */
void Swarm::evaluatePopulationPSOPSO(unsigned generation)
{
    initStatistics(Gen);
    ...
    for (unsigned i = 0; i < tPop; ++i) {
        ...
        if (generation == 0 || population[i].fitness > bestIndividualExp[i].fitness)
            copyParticle(population[i], bestIndividualExp[i]);

        if (population[i].fitness > sBest.fitness)
            copyParticle(population[i], sBest);

        if (i == 0) {
            copyParticle(population[i], Gen.best);
            copyParticle(population[i], Gen.worst);
        } else {
            if (population[i].fitness > Gen.best.fitness)
                copyParticle(population[i], Gen.best);
            if (population[i].fitness < Gen.worst.fitness)
                copyParticle(population[i], Gen.worst);
        }

        Gen.meanFitness    += population[i].fitness / tPop;
        Gen.squaredFitness += std::pow(population[i].fitness, 2.0) / tPop;
    }
 ...
 *   - Multi-evolutionary phase (singlePhase == false): standard
 *     PSO using pbest and this swarm's own sBest, i.e. eq. (1)/(2)
 *     of the paper with sBest playing the role of "pg".
 *   - Single-evolutionary phase (singlePhase == true): adds a
 *     third term pulling toward gBest, i.e. eq. (11)/(12).
 *
 * The discrete (binary / integer-cardinality) position-update
 * rules mirror Swarm::PSOAlgorithm()'s sigmoid-mapped scheme,
 * generalised from two candidate experiences (social/individual)
 * to up to three (sBest/gBest/pbest) -- the paper itself only
 * specifies the continuous case, so the extra branch is this
 * codebase's discrete analogue, not something taken from the
 * paper verbatim. */
void Swarm::PSOAlgorithmPSOPSO(unsigned generation, bool isFirstGeneration,
                                bool singlePhase, const Particle& gBest, double phi3)
{
    ...
    for (unsigned i = 0; i < tPop; ++i) {
        for (unsigned d = 0; d < nAllele; ++d) {
            const double phi1p = rndF() * phi1;              /* toward pbest  */
            const double phi2p = rndF() * phi2;              /* toward sBest  */
            const double phi3p = singlePhase ? rndF() * phi3 : 0.0; /* toward gBest */

            if (isFirstGeneration) {
                if      (!sBest.chromX[d] && !bestIndividualExp[i].chromX[d])
                    population[i].vi[d] = -vMax;
                else if ( sBest.chromX[d] &&  bestIndividualExp[i].chromX[d])
                    population[i].vi[d] =  vMax;
                else
                    population[i].vi[d] =  0.0;
                ...
                population[i].vi[d] +=
                    phi1p * static_cast<double>(bestIndividualExp[i].chromX[d]);
                population[i].vi[d] +=
                    phi2p * static_cast<double>(sBest.chromX[d]);
            } else {
                population[i].vi[d] +=
                    phi1p * (static_cast<double>(bestIndividualExp[i].chromX[d])
                           - static_cast<double>(population[i].chromX[d]));
                ...
                if (singlePhase)
                    population[i].vi[d] +=
                        phi3p * (static_cast<double>(gBest.chromX[d])
                               - static_cast<double>(population[i].chromX[d]));
            }
            ...
            switch (representation) {
                ...
                case INTEGER_A:
                    population[i].chromX[d] = flip(vNorm)
                        ? (singlePhase && flip(0.5) ? gBest.chromX[d] : sBest.chromX[d])
                        : population[i].chromX[d];
                    break;

                case INTEGER_B:
                    if (flip(vNorm)) {
                        population[i].chromX[d] = sBest.chromX[d];
                    } else if (flip(1.0 - vNorm)) {
                        population[i].chromX[d] = (singlePhase && flip(0.5))
                            ? gBest.chromX[d]
                            : bestIndividualExp[i].chromX[d];
                    }
                    /* else: keep current position, as in PSOAlgorithm() */
                    break;
            }
        }
    }
