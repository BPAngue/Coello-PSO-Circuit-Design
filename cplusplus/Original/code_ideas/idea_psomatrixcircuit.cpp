...
void Swarm::initBounds()
{
    ...
    lInf.assign(nVar, 0u); // lower bounds are always 0
    lSup.assign(nVar, 0u); // upper bounds depend on the variable type (input1, input2, gateType)

    for (unsigned cell = 0; cell < decoder.tMat; ++cell) {
        const unsigned column = cell / decoder.numRows;

        /*
         * Number of sources that exist before this
         * column
         */
        const unsigned sourceCount = circuit.numInputs + column * decoder.numRows;
         ...
         *   cell*3 + 2 -> gate type
         */
        const unsigned input1Var = cell * 3;
        const unsigned input2Var = cell * 3 + 1;
        const unsigned gateVar = cell * 3 + 2;
         ...
         */
        if (sourceCount > 0) {
            lSup[input1Var] = sourceCount - 1;
            lSup[input2Var] = sourceCount - 1;
        }

        lSup[gateVar] = circuit.numGates > 0 ? circuit.numGates - 1 : 0;
    }
    ...
    if (representation == BINARY) {
        ...
        for (unsigned i = 0; i < nVar; ++i) {
            const unsigned maxValue = lSup[i];
    ...
    //     lInf[i] = 0;
    //     if (i + 1 % 3) {
    //         lSup[i] = decoder.numRows - 1; // input1 or input2
    //     } else {
    //         lSup[i] = circuit.numGates - 1; // gateType
    //     }
    // }
...
void Swarm::evaluatePopulation(unsigned gen)
{
    ...
    for (unsigned i = 0; i < tPop; ++i) {
        evaluateParticle(population[i]); // evaluate per particle
...
void Swarm::evaluateParticle(Particle& par)
{
    ...
    par.numNoGates = 0; // number of WIRE inputs (zero if the solution is not perfect)
