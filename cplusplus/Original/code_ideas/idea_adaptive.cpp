...
double calculateMigrationProbability(double transmitterFitness, double globalBestFitness, double temperature) {
    if (temperature <= 0.0) {
        return 0.0;
    } // ask about this

    /* globalBest is better than transmitter */
    const double delta = globalBestFitness - transmitterFitness;
...
MigrationStrategy chooseStrategy(double transmitterFitness, double globalFitness, double temperature, double& probability) {
    if (transmitterFitness >= globalFitness) {
        probability = 1.0;
        return MigrationStrategy::BestWorst;
    }

    /* Otherwise calculate probability of best-random */
    probability = calculateMigrationProbability(transmitterFitness, globalFitness, temperature);
...
void runIsland(unsigned islandIndex, PSwarm::Swarm& swarm, unsigned migrationInterval, unsigned migrationRate, 
               bool adaptive, std::atomic<double>& sharedTemperature, Barrier& migrationBarrier, 
    ...
    for (unsigned run = 0; run < swarm.nRun; ++run) {
        ...
        for (unsigned gen = 0; gen < swarm.nGen; ++gen) {
            ...
            if (migrationTime) {
                ...
                double globalBestFitness = islands[0]->Gen.best.fitness;

                for (unsigned i = 1; i < numIslands; ++i) {
                    if (islands[i]->Gen.best.fitness > globalBestFitness) {
                        globalBestFitness = islands[i]->Gen.best.fitness;
                    }
                }
                ...
                /* check if adaptive */
                if (adaptive) {
                    strategy = chooseStrategy(transmitterFitness, globalBestFitness, sharedTemperature.load(), probability);
                }
                ...
                if (adaptive && islandIndex == 0) {
                    const double alpha = rndF();
                    double newTemperature = sharedTemperature.load() * alpha;

                    /* Avoid numerical zero */
                    if (newTemperature < 1.0e-12) {
                        newTemperature = 1.0e-12;
                    }

                    sharedTemperature.store(newTemperature);
                }
...
int main(int argc, char* argv[])
{
    ...
    /* Shared temperature, one value for the whole run */
    std::atomic<double> sharedTemperature{initialTemperature};
    ...
    for (unsigned i = 0; i < numIslands; ++i) {
        workers.emplace_back(PSwarm::runIsland, i, std::ref(*islands[i]), migrationInterval, migrationRate, adaptive, std::ref(sharedTemperature), std::ref(migrationBarrier), std::ref(migrationBuffers), std::ref(islands));
    }
