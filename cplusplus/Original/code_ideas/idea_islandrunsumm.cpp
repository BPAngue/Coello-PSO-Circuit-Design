...
/* -------------------------------------------------------
 * writeIslandSummaryHeader
 * Header for the cross-island summary CSV. Deliberately
 * NOT added to statistics.h/.cpp (those stay untouched) --
 * globalHeader() there hardcodes "Run" as the leading
 * column label, so this writes the equivalent header text
 * directly, swapping in "Island". The per-row data itself
 * still goes through the existing runStatistics() below.
 * ------------------------------------------------------- */
void writeIslandSummaryHeader(const std::string& filename, const Swarm& swarm)
{
    ...
    std::fprintf(pf, "Statistics of the results obtained across all islands (Island Model PSO)\n\n");

    std::fprintf(pf, "     Processed File:,   %s\n", swarm.nfInput.c_str());
    std::fprintf(pf, "   Population Size:,   %u\n",  swarm.tPop);
    std::fprintf(pf, "Number of Generations:,   %u\n", swarm.nGen);
    std::fprintf(pf, "     Runs per Island:,   %u\n", swarm.nRun);
    ...
    std::fprintf(pf, "   Representation:, %s\n",
        swarm.representation == BINARY    ? "Binary" :
        swarm.representation == INTEGER_A ? "Integer A" : "Integer B");

    std::fprintf(pf, "\n");
    std::fprintf(pf, "Island,Mean Fitness,Generation,");
...
void runIsland(unsigned islandIndex, PSwarm::Swarm& swarm, unsigned migrationInterval, unsigned migrationRate, 
               ...
               const std::string& islandSummaryFile) {
    ...
    for (unsigned run = 0; run < swarm.nRun; ++run) {
        ...
        /* Roll this run's result into the island-level aggregate */
        islandStats.meanFitness    += swarm.Run.meanFitness    / swarm.nRun;
        islandStats.squaredFitness += swarm.Run.squaredFitness / swarm.nRun;
    ...
    /* Append this island's aggregated row to the shared summary CSV,
       reusing the existing runStatistics() from statistics.cpp
       unmodified. runStatistics() reads its data from swarm.Run, so
       the aggregated islandStats is loaded into swarm.Run first; this
       island's runs are already finished at this point, so overwriting
       swarm.Run here has no further effect on this thread. */
    swarm.Run.meanFitness    = islandStats.meanFitness;
    swarm.Run.squaredFitness = islandStats.squaredFitness;
    swarm.Run.generation     = islandStats.generation;
    swarm.copyParticle(islandStats.best,  swarm.Run.best);
    swarm.copyParticle(islandStats.worst, swarm.Run.worst);

    {
        std::lock_guard<std::mutex> lock(islandCsvMutex);
        runStatistics(islandSummaryFile, islandIndex, swarm);
    }
...
int main(int argc, char* argv[])
{
    ...
    /* Cross-island summary CSV: one row per island, appended once that
       island has completed all of its runs. Header written once, up
       front, from the main thread before any island writes a row. */
    const std::string islandSummaryFile = "csvs/resultIslandSumm.csv";
    PSwarm::writeIslandSummaryHeader(islandSummaryFile, *islands[0]);
    ...
    for (unsigned i = 0; i < numIslands; ++i) {
        workers.emplace_back(PSwarm::runIsland, i, std::ref(*islands[i]), migrationInterval, migrationRate, adaptive, initialTemperature, std::ref(migrationBarrier), std::ref(migrationBuffers), std::ref(islands), std::cref(islandSummaryFile));
    }
